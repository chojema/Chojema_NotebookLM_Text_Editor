...
I18N = {
    "ko": {
        ...
        "usage_video": "사용법 설명 영상",
        "canvas_warning_0": "* PowerPoint(파워포인트)나 기타 작업을 위한 애플리케이션을 열어 두고, 이곳에서 수정 작업을 하면서 해당 이미지나 필요한 텍스트를 작업 중인 애플리케이션으로 복사해 사용하는 방식으로 진행하기를 추천합니다.",
        "canvas_warning": "* 텍스트 추출이나 배경 복원 단추를 눌러도 단추 아래에 작업 중이라는 안내 메시지가 나오지 않고 제대로 작동하지 않는다면 잠깐 기다렸다 다시 시도해 보세요.",
        "canvas_warning_2": "* 앱 설치 후 최초 실행 시에는 필요한 모델 다운로드나 메모리 탑재 등으로 시간이 좀 더 걸릴 수 있습니다.",
        "canvas_warning_3": "* 작업 파일은 특정 서버로 전송되는 게 아니라 자신의 웹 브라우저 안에서 처리됩니다.",
        "canvas_warning_4": "* 이 앱은 자동 업데이트되지 않으므로, 가끔 좌측 창 하단의 링크를 이용해 GitHub의 앱 페이지를 방문하여 최신 버전이 있는지 확인하고 적용하기 바랍니다.",
    },
    "en": {
        ...
        "usage_video": "Usage Guide Video",
        "canvas_warning_0": "* It is recommended to keep PowerPoint or other work applications open and copy the edited images or text into them as you work.",
        "canvas_warning": "* If the action doesn't start or shows no progress message after clicking, please wait a moment and try again.",
        "canvas_warning_2": "* The first run after installation may take longer due to model downloads and memory loading.",
        "canvas_warning_3": "* Your files are not sent to an external server; they are processed locally within your system.",
        "canvas_warning_4": "* This app does not update automatically. Please occasionally visit the GitHub page via the sidebar link to check for new versions.",
    },
}
...
/* Hide trigger buttons used for JS callbacks */
div[class*="st-key-del_trigger_"] {
  display: none !important;
}
</style>
""",
...
def rect_to_selection(rect, img_w, img_h, canvas_w, canvas_h):
    """
    canvas_w, canvas_h: current canvas display size (zoomed)
    img_w, img_h: original image size
    rect: fabric.js object (dict)
    """
    ...
    l = rect.get("left", 0.0)
    t = rect.get("top", 0.0)
    w = rect.get("width", 0.0) * rect.get("scaleX", 1.0)
    h = rect.get("height", 0.0) * rect.get("scaleY", 1.0)
    ...
    # Normalize (ensure left < right, top < bottom) and clip to image bounds
    final_left = max(0.0, min(x1, x2))
    final_top = max(0.0, min(y1, y2))
    final_right = min(float(img_w), max(x1, x2))
    final_bottom = min(float(img_h), max(y1, y2))

    return {
        "left": final_left,
        "top": final_top,
        "width": max(1.0, final_right - final_left),
        "height": max(1.0, final_bottom - final_top),
    }
...
def extract_selection(canvas_json, img_w, img_h, canvas_w, canvas_h):
    ...
    rect = None
    # Find the most recently added rectangle
    ...
    return rect_to_selection(rect, img_w, img_h, canvas_w, canvas_h)
...
<div class="sidebar-footer">
  ...
  <a href="https://github.com/chojema/Chojema_NotebookLM_Text_Editor" target="_blank">GitHub</a>
</div>
""",
    unsafe_allow_html=True,
)
...
canvas_col, panel_col = st.columns([4.4, 1.9])
...
with canvas_col:
    ...
    st.markdown(
        f"""
        <div style="font-size: 13px; color: #6f7482; margin-top: 10px; padding: 12px; background: #f8fafc; border-radius: 10px; border: 1px solid #e2e8f0; line-height: 1.6;">
          <div style="margin-bottom: 4px; color: #0f766e; font-weight: 600;">{t("canvas_warning_0")}</div>
          <div style="margin-bottom: 4px;">{t("canvas_warning")}</div>
          <div style="margin-bottom: 4px;">{t("canvas_warning_2")}</div>
          <div style="margin-bottom: 4px;">{t("canvas_warning_3")}</div>
          <div>{t("canvas_warning_4")}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with panel_col:
    ...
    history = st.session_state.get("extracted_history", ["", "", ""])
    for idx in range(3):
        row_text_col, row_btn_col = st.columns([8.8, 1.2], gap="small")
        with row_text_col:
        ...
        with row_btn_col:
            ...
            copy_tooltip = t("copy_text")
            delete_tooltip = "텍스트 삭제" if st.session_state.lang == "ko" else "Delete Text"
...
<style>
  ...
  button {{
    ...
    background: #f0f2f6; border: 1px solid #d0d4da; border-radius: 6px;
    cursor: pointer; font-size: 16px; color: #31333f; font-family: Segoe UI, sans-serif;
    display: flex; align-items: center; justify-content: center;
    box-sizing: border-box;
  }}
  button:hover {{ background: #e2e4e9; }}
</style>
<div class="icon-container">
  <button id="clip-text-btn-{idx}" onclick="copyTextToClipboard{idx}()" title="{copy_tooltip}">
    ⎘
  </button>
  <button id="del-text-btn-{idx}" onclick="deleteText{idx}()" title="{delete_tooltip}">
    ✕
  </button>
</div>
<script>
...
function deleteText{idx}() {{
  var b = window.parent.document.querySelector('.st-key-del_trigger_{idx} button');
  if(b) b.click();
}}
</script>
"""
            st.components.v1.html(text_icon_html, height=85)
            # Hidden streamlit buttons to trigger state update for deletion
            if st.button("del", key=f"del_trigger_{idx}"):
                st.session_state[f"text_history_{idx}"] = ""
                st.rerun()
